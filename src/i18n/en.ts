export const en = {
  // Data location
  dataSetupTitle: 'Choose where to save your tasks',
  dataSetupMigrationHint: 'Your existing tasks were found. Choose a folder with enough space and Desklist will copy them safely.',
  dataSetupNewHint: 'Choose a folder for your tasks and reminders.',
  chooseDataFolder: 'Choose folder',
  confirmDataFolder: 'Confirm and continue',
  migratingData: 'Moving data safely...',
  dataSetupSafetyNote: 'Your original data stays untouched until the move is verified.',

  // TitleBar
  settings: 'Settings',
  minimizeToTray: 'Minimize to tray',
  closeToTray: 'Close to tray',

  // CalendarView
  switchToCalendar: 'Switch to Calendar',
  switchToList: 'Switch to List',
  calendarWeekdays: ['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'] as string[],
  calendarNoEvents: 'No pending events',
  calendarSelectHint: 'Tap a date to view events',

  // EventList
  tabToday: 'Today',
  tabIncomplete: 'Incomplete',
  tabCompleted: 'Completed',
  tabAll: 'All',
  loading: 'Loading...',
  noEvents: 'No events',

  // EventForm
  editEvent: 'Edit Event',
  newEvent: 'New Event',
  fieldTitle: 'Title',
  fieldDescription: 'Description',
  fieldTime: 'Time',
  fieldScheduledStart: 'Scheduled start',
  fieldScheduledEnd: 'Scheduled end (optional)',
  fieldDueTime: 'Due time (optional)',
  fieldReminder: 'Reminder',
  fieldRecurrence: 'Recurrence',
  fieldRecurrenceEnd: 'End Date',
  titlePlaceholder: 'Event title',
  descPlaceholder: 'Description (optional)',
  recurrenceNone: 'No repeat',
  recurrenceDaily: 'Daily',
  recurrenceWeekly: 'Weekly',
  recurrenceMonthly: 'Monthly',
  cancel: 'Cancel',
  save: 'Save',
  close: 'Close',

  // EventCard
  markIncomplete: 'Mark as incomplete',
  markComplete: 'Mark as complete',
  deleteEvent: 'Delete event',

  // ReminderConfig
  remindOnTime: 'Due reminder',
  advanceReminder: 'Advance reminder',
  noReminder: 'No reminder',
  min5Before: '5 min before',
  min15Before: '15 min before',
  min30Before: '30 min before',
  hour1Before: '1 hour before',
  day1Before: '1 day before',

  // SettingsPanel
  settingsTitle: 'Settings',
  dataLocation: 'Task data location (click to open)',
  agentAccess: 'AI / Agent access',
  agentAccessDescription: 'Give this endpoint and token only to trusted local AI tools. Do not share the token publicly.',
  agentEndpoint: 'Endpoint',
  agentMcpEndpoint: 'MCP endpoint (for Codex)',
  agentToken: 'Access token',
  showToken: 'Show',
  hideToken: 'Hide',
  copyToken: 'Copy',
  copiedToken: 'Copied',
  regenerateToken: 'Regenerate token',
  regenerateTokenConfirm: 'Regenerating immediately invalidates tokens already configured in AI tools. Continue?',
  windowOpacity: 'Window Opacity',
  themeColor: 'Theme Color',
  defaultReminderLabel: 'Default Reminder',
  defaultReminderCheck: 'Enable due reminder by default',
  resetDefaults: 'Reset Defaults',
  done: 'Done',
  language: 'Language',
};
